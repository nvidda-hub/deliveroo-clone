...
import { selectedBasketItems, removeFromBasket, basketTotal } from '../reduxSlices/basketSlice'
    ...
    <SafeAreaView className=" mt-10 bg-white">
      <View className="bg-gray-100">
        <View className="flex-row items-center bg-white border-b border-[#00CCBB] shadow-xs py-2">
          ...
          <TouchableOpacity onPress={navigation.goBack} className="rounded-full bg-[#00CCBB] absolute top-3 right-5">
        ...
        <View className="flex-row bg-white items-center space-x-4 px-4 py-3 my-5">
          ...
          />
          <Text className="flex-1">Deliver in 45-50 Mins</Text>
          <TouchableOpacity>
            <Text className="text-[#00CCBB]">Change</Text>
          </TouchableOpacity>
        </View>
        <ScrollView className>
          {Object.keys(groupedItems).length >= 1 ? Object.entries(groupedItems).map(([key, items]) => {
            return <View key={key} className="bg-white px-3 py-1 flex-row justify-between items-center ">

              <Text className="flex-1 text-[#00CCBB]">{items.length} x</Text>
              <Image 
                  source={{uri:urlFor(items[0]?.imageUrl).url()}}
                  className="w-12 h-12 rounded-full"
              />
              <Text className="flex-1 ml-2">{items[0]?.name}</Text>
              <Text className="text-gray-500 text-lg font-bold flex justify-end">₹ {items[0]?.price} /-</Text>
          <TouchableOpacity
          onPress={() => dispatch(removeFromBasket({id:key}))}>
            <Text className="text-[#00CCBB] text-xs ml-2">
              Remove
            </Text>
          </TouchableOpacity>
              </View>
        ...
        <View className="bg-white p-5 mt-32 w-full space-y-4">
           <View className="flex-row  justify-between">
			  <Text className="text-gray-500 text-xs font-bold">Subtotal</Text>
			  <Text className="text-gray-500 text-xs font-bold">₹ {basketTotalForItems} /-</Text>
		   </View>
           <View className="flex-row text-gray-500 text-xs justify-between">
			  <Text className="text-gray-500 text-xs font-bold">Delivery fee</Text>
			  <Text className="text-gray-500 text-xs font-bold">₹ 49 /-</Text>
		   </View>
           <View className="flex-row justify-between font-extrabold">
			  <Text className="font-extrabold">Order Total</Text>
			  <Text className="font-extrabold">₹ {basketTotalForItems+49} /-</Text>
		   </View>
		   <TouchableOpacity>
			<Text className="bg-[#00CCBB] text-xl px-5 py-3 flex text-center text-white font-extrabold mx-3 rounded-lg">Place order</Text>
		   </TouchableOpacity>
        </View>
      </View>
    </SafeAreaView>
